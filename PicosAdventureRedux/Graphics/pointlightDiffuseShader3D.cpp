...
void PointlightDiffuseShader3DClass::setPositions(XMFLOAT4 pos1, XMFLOAT4 pos2, XMFLOAT4 pos3, XMFLOAT4 pos4, XMFLOAT4 pos5)
{
	lightPos1_ = pos1;
	lightPos2_ = pos2;
	lightPos3_ = pos3;
...
bool PointlightDiffuseShader3DClass::setupShader(ID3D11Device* device, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	// Create the constant buffer pointer so we can access the vertex shader constant buffer from within this class.
	result = device->CreateBuffer(&lightPositionBufferDesc, NULL, &lightPositionBuffer_);
	if(FAILED(result))
	{
		return false;
	}
	...
	result = device->CreateBuffer(&lightColorBufferDesc, NULL, &lightColorBuffer_);
	if(FAILED(result))
	{
		return false;
	}
					                    ...
					                    XMFLOAT4X4 projectionMatrix, ID3D11ShaderResourceView** textureArray, LightClass* light)
{
	...
	dataPtr3->lightPosition[3] = lightPos4_;
	dataPtr3->lightPosition[4] = lightPos5_;
	...
	dataPtr4->diffuseColor[3] = XMFLOAT4(0.1f, 0.1f, 0.1f, 1.0f);
	dataPtr4->diffuseColor[4] = XMFLOAT4(0.1f, 0.1f, 0.1f, 1.0f);
