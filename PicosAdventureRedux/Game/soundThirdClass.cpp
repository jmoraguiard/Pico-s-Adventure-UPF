#include "soundThirdClass.h"

SoundThirdClass::SoundThirdClass()
{
	m_DirectSound = 0;
	m_primaryBuffer = 0;
...
SoundThirdClass::SoundThirdClass(const SoundThirdClass& other)
{
...
bool SoundThirdClass::setup(HWND hwnd)
{
	bool result;
	...
	result = loadFile("change_level", "change_level");
	if(!result)
	{
		MessageBoxA(NULL, "Could not load change_level", "SoundSecondClass - Error", MB_ICONERROR | MB_OK);
		return false;
	}
...
bool SoundThirdClass::setupDirectSound(HWND hwnd)
{
	HRESULT result;
...
void SoundThirdClass::destroyDirectSound()
{
...
bool SoundThirdClass::loadFile(std::string name, std::string fileName)
{
	bool result;
...
bool SoundThirdClass::loadWaveFile(std::string filename, IDirectSoundBuffer8** secondaryBuffer)
{
	int error;
	FILE* filePtr;
	unsigned int count;
...
void SoundThirdClass::destroyWaveFile(IDirectSoundBuffer8** secondaryBuffer)
{
...
bool SoundThirdClass::playFile(std::string name, bool loop)
{
	HRESULT result;
...
bool SoundThirdClass::isPlaying(std::string name)
{
	HRESULT result;
	IDirectSoundBuffer8* sound = sounds_.at(name);
	DWORD status;
...
bool SoundThirdClass::playCelebrate()
{
	HRESULT result;
...
bool SoundThirdClass::playPointing()
{
	HRESULT result;
...
bool SoundThirdClass::playSurprise()
{
	HRESULT result;
